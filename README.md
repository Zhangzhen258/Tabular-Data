...
## 二、Tabular Foundation Model

### 1. [TabICL: A Tabular Foundation Model for In-Context Learning on Large Data](https://arxiv.org/abs/2502.05564)
作者提出了一个新的TFMs，相对于TabPFN范式，作者改进如下：作者首先按照列来读取特征，并使用set transformer来对每个列的特征进行建模。随后，作者使用四个查询token来对特征进行查询注意力机制，将每个样本的特征压缩为四个token的长度。最后，按照与TabPFN相同的注意力机制进行样本间的特征建模。

### 2. [TabICLv2: A better, faster, scalable, and open tabular foundation model](https://arxiv.org/abs/2602.11139)
作者在TabICL的基础上对框架进行了改进：（1）在读取特征时，作者采用了特征重复策略，按照（0,1,3）的顺序将特征分为一组，如果有m个特征m组，每组内有3个特征。（2）为了更好的读取标签特征，作者将标签特征加到每一组的特征中。（3）作者在set transformer和样本间注意力中引入了QASSMax，去注意力中查询q进行了放缩。（4）更丰富的预训练预训练。
